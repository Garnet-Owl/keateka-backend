...
from app.features.payments.models import Payment, PaymentStatus, PaymentProvider
from app.features.payments.exceptions import PaymentProcessingError, InvalidPaymentStateError
from app.features.payments.schemas import PaymentCreate, MPESAPaymentCreate
...
def prepare_test_payment():
    def step(context):
        ...
        context.test_mpesa_payment_data = MPESAPaymentCreate(
            amount=test_amount, job_id=1, provider=PaymentProvider.MPESA, currency="KES", phone_number="254712345678"
        )
...
class TestPaymentProcessor:
    ...
    async def test_process_mpesa_payment_invalid_state_fails(self):
        ...
        with given([prepare_db(), prepare_test_payment(), prepare_mpesa_client()]) as context:
            ...
            with when("processing payment in invalid state"), pytest.raises(InvalidPaymentStateError) as exc_info:
    ...
    async def test_mpesa_api_error_handling(self):
        ...
        with given([prepare_test_payment()]) as context:
            with patch("httpx.AsyncClient") as mock_client:
                ...
                with when("making API call that fails"), pytest.raises(PaymentProcessingError) as exc_info:
