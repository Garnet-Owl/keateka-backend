...
class PaymentService:
    ...
    async def create_payment(self, payment_data: PaymentCreate, user_id: int, reference: str) -> Payment:
        ...
        payment = Payment(reference=reference, user_id=user_id, created_at=datetime.now(UTC), **payment_dict)
    ...
    async def initiate_mpesa_payment(self, payment: Payment, phone_number: str) -> dict:
        ...
        try:
            ...
            await self.db.commit()
            raise HTTPException(
                status_code=status.HTTP_502_BAD_GATEWAY, detail=f"Failed to initiate M-PESA payment: {str(e)}"
            )
